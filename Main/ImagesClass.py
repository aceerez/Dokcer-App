...
class Images:
    def __init__(self, master):
        ...
        self.imagLodlistBtn = tk.Button (self.imagInfoFrame, text="Load Images List", command=lambda: self.GetImageList ())
    ...
    def RunImage(self):
        ...
        os.system ('docker run -it  {0} bash'.format (imageID))
    ...
    def CreatImagePopMenu(self):
        ...
        print(self.GetImageID())
        if self.GetImageID() != "":
            self.imagPopMenu.add_command (label="create image(New Docker)", command=self.RunImage ())
            self.imagPopMenu.add_command (label="download image", command=self.DownloadImage)
         #   self.imagPopMenu.add_command (label="run image", command=self.DeleteContainer)
          #  self.imagPopMenu.add_command (label="delete image", command=self.DeleteContainer)
